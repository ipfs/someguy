...
- JSON delegated routing requests now surface close to the requested number of providers. Previously, records without addresses were filtered out after the upstream cap, so callers received far fewer than `--records-limit`.
